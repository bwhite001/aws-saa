...
- 1. Enable AWS Multi-Factor Authentication (AWS MFA) for privileged users(Correct)
...
- 1. Migrate the website to Amazon S3. Use S3 cross-region replication (S3 CRR) between AWS Regions in the US and Asia
- 2. Leverage a Amazon Route 53 geo-proximity routing policy pointing to on-premises servers
- 3. Use Amazon CloudFront with a custom origin pointing to the DNS record of the website on Amazon Route 53
- 4. Use Amazon CloudFront with a custom origin pointing to the on-premises servers(Correct)
...
- 1. The spreadsheet on the Amazon Elastic File System (Amazon EFS) can be accessed in other AWS regions by using an inter-region VPC peering connection(Correct)
- 2. The spreadsheet will have to be copied into Amazon EFS file systems of other AWS regions as Amazon EFS is a regional service and it does not allow access from other AWS regions
...
- 4. The spreadsheet will have to be copied in Amazon S3 which can then be accessed from any AWS region

*Correct Answer(s):*
 1. The spreadsheet on the Amazon Elastic File System (Amazon EFS) can be accessed in other AWS regions by using an inter-region VPC peering connection
...
- 1. Amazon FSx for Lustre(Correct)
- 2. AWS Glue
...
- 1. The junior scientist only needs to pay S3TA transfer charges for the image upload
- 2. The junior scientist only needs to pay Amazon S3 transfer charges for the image upload
- 3. The junior scientist needs to pay both S3 transfer charges and S3TA transfer charges for the image upload
- 4. The junior scientist does not need to pay any transfer charges for the image upload(Correct)
...
- 1. Multi-AZ follows synchronous replication and spans at least two Availability Zones (AZs) within a single region. Read replicas follow asynchronous replication and can be within an Availability Zone (AZ), Cross-AZ, or Cross-Region(Correct)
- 2. Multi-AZ follows asynchronous replication and spans one Availability Zone (AZ) within a single region. Read replicas follow synchronous replication and can be within an Availability Zone (AZ), Cross-AZ, or Cross-Region
- 3. Multi-AZ follows asynchronous replication and spans at least two Availability Zones (AZs) within a single region. Read replicas follow synchronous replication and can be within an Availability Zone (AZ), Cross-AZ, or Cross-Region
...
*Correct Answer(s):*
 1. Multi-AZ follows synchronous replication and spans at least two Availability Zones (AZs) within a single region. Read replicas follow asynchronous replication and can be within an Availability Zone (AZ), Cross-AZ, or Cross-Region
...
- 1. Enable Multi Factor Authentication (MFA) for the AWS account root user account(Correct)
- 2. Encrypt the access keys and save them on Amazon S3
- 3. Create a strong password for the AWS account root user(Correct)
- 4. Send an email to the business owner with details of the login username and password for the AWS root user. This will help the business owner to troubleshoot any login issues in future
- 5. Create AWS account root user access keys and share those keys only with the business owner

*Correct Answer(s):*
 1. Enable Multi Factor Authentication (MFA) for the AWS account root user account3. Create a strong password for the AWS account root user
...
- 1. Amazon API Gateway, Amazon Simple Queue Service (Amazon SQS) and Amazon Kinesis(Correct)
- 2. Elastic Load Balancer, Amazon Simple Queue Service (Amazon SQS), AWS Lambda
- 3. Amazon Gateway Endpoints, Amazon Simple Queue Service (Amazon SQS) and Amazon Kinesis
- 4. Amazon Simple Queue Service (Amazon SQS), Amazon Simple Notification Service (Amazon SNS) and AWS Lambda

*Correct Answer(s):*
 1. Amazon API Gateway, Amazon Simple Queue Service (Amazon SQS) and Amazon Kinesis
...
- 2. Create multiple AWS Direct Connect connections between the AWS Cloud and branch offices in Europe and Asia. Use the direct connect connections for faster file uploads into Amazon S3
- 3. Create multiple AWS Site-to-Site VPN connections between the AWS Cloud and branch offices in Europe and Asia. Use these VPN connections for faster file uploads into Amazon S3
- 4. Use multipart uploads for faster file uploads into the destination Amazon S3 bucket(Correct)
- 5. Use Amazon S3 Transfer Acceleration (Amazon S3TA) to enable faster file uploads into the destination S3 bucket(Correct)

*Correct Answer(s):*
 4. Use multipart uploads for faster file uploads into the destination Amazon S3 bucket5. Use Amazon S3 Transfer Acceleration (Amazon S3TA) to enable faster file uploads into the destination S3 bucket
...
- 1. Push score updates to an Amazon Simple Notification Service (Amazon SNS) topic, subscribe an AWS Lambda function to this Amazon SNS topic to process the updates and then store these processed updates in a SQL database running on Amazon EC2 instance
- 2. Push score updates to Amazon Kinesis Data Streams which uses an AWS Lambda function to process these updates and then store these processed updates in Amazon DynamoDB(Correct)
...
- 4. Push score updates to an Amazon Simple Queue Service (Amazon SQS) queue which uses a fleet of Amazon EC2 instances (with Auto Scaling) to process these updates in the Amazon SQS queue and then store these processed updates in an Amazon RDS MySQL database
...
- 3. Create a new IAM role with the required permissions to access the resources in the production environment. The users can then assume this IAM role while accessing the resources from the production environment(Correct)
- 4. Both IAM roles and IAM users can be used interchangeably for cross-account access

*Correct Answer(s):*
 3. Create a new IAM role with the required permissions to access the resources in the production environment. The users can then assume this IAM role while accessing the resources from the production environment
...
- 1. Use Amazon EC2 reserved instances to run the workflow processes
- 2. Use Amazon EC2 on-demand instances to run the workflow processes
- 3. Use Amazon EC2 spot instances to run the workflow processes(Correct)
...
- 1. Both Amazon ECS with EC2 launch type and Amazon ECS with Fargate launch type are charged based on vCPU and memory resources that the containerized application requests
...
- 3. Amazon ECS with EC2 launch type is charged based on EC2 instances and EBS volumes used. Amazon ECS with Fargate launch type is charged based on vCPU and memory resources that the containerized application requests(Correct)
- 4. Both Amazon ECS with EC2 launch type and Amazon ECS with Fargate launch type are charged based on Amazon EC2 instances and Amazon EBS Elastic Volumes used

*Correct Answer(s):*
 3. Amazon ECS with EC2 launch type is charged based on EC2 instances and EBS volumes used. Amazon ECS with Fargate launch type is charged based on vCPU and memory resources that the containerized application requests
...
- 1. Contact AWS support to retrieve the AWS KMS key from their backup
- 2. The company should issue a notification on its web application informing the users about the loss of their data
- 3. As the AWS KMS key was deleted a day ago, it must be in the 'pending deletion' status and hence you can just cancel the KMS key deletion and recover the key(Correct)
- 4. The AWS KMS key can be recovered by the AWS root account user
...
- 2. Use Amazon GuardDuty to monitor any malicious activity on data stored in Amazon S3. Use security assessments provided by Amazon Inspector to check for vulnerabilities on Amazon EC2 instances(Correct)
- 3. Use Amazon GuardDuty to monitor any malicious activity on data stored in Amazon S3. Use security assessments provided by Amazon GuardDuty to check for vulnerabilities on Amazon EC2 instances
...
*Correct Answer(s):*
 2. Use Amazon GuardDuty to monitor any malicious activity on data stored in Amazon S3. Use security assessments provided by Amazon Inspector to check for vulnerabilities on Amazon EC2 instances
...
- 2. Amazon S3 Intelligent-Tiering => Amazon S3 Standard(Correct)
- 3. Amazon S3 Standard => Amazon S3 Intelligent-Tiering
- 4. Amazon S3 Standard-IA => Amazon S3 Intelligent-Tiering
- 5. Amazon S3 One Zone-IA => Amazon S3 Standard-IA(Correct)

*Correct Answer(s):*
 2. Amazon S3 Intelligent-Tiering => Amazon S3 Standard5. Amazon S3 One Zone-IA => Amazon S3 Standard-IA
...
- 1. Change the application architecture to create a new Amazon S3 bucket for each day's data and then upload the daily files directly under that day's bucket
...
- 3. Change the application architecture to create customer-specific custom prefixes within the single Amazon S3 bucket and then upload the daily files into those prefixed locations(Correct)
- 4. Change the application architecture to create a new Amazon S3 bucket for each customer and then upload each customer's files directly under the respective buckets
...
- 3. Use Amazon CloudFront distribution in front of the Application Load Balancer(Correct)
- 4. Use Amazon Aurora Replica(Correct)
- 5. Use AWS Direct Connect

*Correct Answer(s):*
 3. Use Amazon CloudFront distribution in front of the Application Load Balancer4. Use Amazon Aurora Replica
...
- 3. Order 10 AWS Snowball Edge Storage Optimized devices to complete the one-time data transfer(Correct)
- 4. Setup AWS Direct Connect to establish connectivity between the on-premises data center and AWS Cloud
- 5. Setup AWS Site-to-Site VPN to establish on-going connectivity between the on-premises data center and AWS Cloud(Correct)

*Correct Answer(s):*
 3. Order 10 AWS Snowball Edge Storage Optimized devices to complete the one-time data transfer5. Setup AWS Site-to-Site VPN to establish on-going connectivity between the on-premises data center and AWS Cloud
...
- 2. VPC Flow Logs, Domain Name System (DNS) logs, AWS CloudTrail events(Correct)
- 3. Amazon CloudFront logs, Amazon API Gateway logs, AWS CloudTrail events
- 4. VPC Flow Logs, Amazon API Gateway logs, Amazon S3 access logs

*Correct Answer(s):*
 2. VPC Flow Logs, Domain Name System (DNS) logs, AWS CloudTrail events
...
- 2. Dynamic content, as determined at request time (cache-behavior configured to forward all headers)(Correct)
- 3. Static content such as style sheets, JavaScript files
- 4. E-commerce assets such as product photos
- 5. Proxy methods PUT/POST/PATCH/OPTIONS/DELETE go directly to the origin(Correct)

*Correct Answer(s):*
 2. Dynamic content, as determined at request time (cache-behavior configured to forward all headers)5. Proxy methods PUT/POST/PATCH/OPTIONS/DELETE go directly to the origin
...
- 1. Use a Amazon DynamoDB global table for the games table and use Amazon DynamoDB tables for the users and games_played tables
- 2. Use a Amazon DynamoDB global table for the games table and use Amazon Aurora for the users and games_played tables
- 3. Use an Amazon Aurora Global Database for the games table and use Amazon Aurora for the users and games_played tables(Correct)
- 4. Use an Amazon Aurora Global Database for the games table and use Amazon DynamoDB tables for the users and games_played tables

*Correct Answer(s):*
 3. Use an Amazon Aurora Global Database for the games table and use Amazon Aurora for the users and games_played tables
...
- 1. Configure a lifecycle policy to transition the objects to Amazon S3 One Zone-Infrequent Access (S3 One Zone-IA) after 7 days
- 2. Configure a lifecycle policy to transition the objects to Amazon S3 Standard-Infrequent Access (S3 Standard-IA) after 30 days
- 3. Configure a lifecycle policy to transition the objects to Amazon S3 Standard-Infrequent Access (S3 Standard-IA) after 7 days
- 4. Configure a lifecycle policy to transition the objects to Amazon S3 One Zone-Infrequent Access (S3 One Zone-IA) after 30 days(Correct)
...
- 1. Use Amazon SQS FIFO (First-In-First-Out) queue to process the messages
- 2. Use Amazon SQS FIFO (First-In-First-Out) queue in batch mode of 4 messages per operation to process the messages at the peak rate(Correct)
- 3. Use Amazon SQS standard queue to process the messages
- 4. Use Amazon SQS FIFO (First-In-First-Out) queue in batch mode of 2 messages per operation to process the messages at the peak rate

*Correct Answer(s):*
 2. Use Amazon SQS FIFO (First-In-First-Out) queue in batch mode of 4 messages per operation to process the messages at the peak rate
